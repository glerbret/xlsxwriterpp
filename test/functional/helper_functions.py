...
def _compare_xlsx_files(got_file, exp_file, ignore_files, ignore_elements):
    ...
    for filename in exp_files:
        ...
        # Collapse empty creator and lastModifiedBy tags
        # Remove empty modified tags
        if filename == "docProps/core.xml":
            exp_xml_str = re.sub(r" ?John", "", exp_xml_str)
            exp_xml_str = re.sub(r"<dc:creator></dc:creator>",
                                 "<dc:creator/>", exp_xml_str)
            exp_xml_str = re.sub(
                r"<cp:lastModifiedBy></cp:lastModifiedBy>", "<cp:lastModifiedBy/>", exp_xml_str)
            exp_xml_str = re.sub(
                r"\d\d\d\d-\d\d-\d\dT\d\d\:\d\d:\d\dZ", "", exp_xml_str
            )
            exp_xml_str = re.sub(
                r"<dcterms:modified xsi:type=\"dcterms:W3CDTF\"></dcterms:modified>", "", exp_xml_str)
            got_xml_str = re.sub(
        ...
        if filename == "xl/workbook.xml":
            exp_xml_str = re.sub(
                r"<workbookView[^>]*>", "<workbookView/>", exp_xml_str)
            got_xml_str = re.sub(
                r"<workbookView[^>]*>", "<workbookView/>", got_xml_str)
        ...
        if re.match(r"xl/worksheets/sheet\d.xml", filename):
            ...
            exp_xml_str = re.sub(
                r'(<pageSetup[^>]*) r:id="rId1"', r"\1", exp_xml_str)
        ...
        if filename in ignore_elements:
            ...
            for pattern in patterns:
                exp_xml = [
                    tag for tag in exp_xml if not re.match(pattern, tag)]
                got_xml = [
                    tag for tag in got_xml if not re.match(pattern, tag)]
